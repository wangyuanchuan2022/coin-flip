...
const results = { heads: 0, tails: 0, dropped: 0 };
const byPower = [
  { heads: 0, tails: 0 },
  { heads: 0, tails: 0 },
  { heads: 0, tails: 0 },
...
console.log(`trials=${N} settled=${total} dropped=${results.dropped}`);
...
byPower.forEach((s, i) => {
  console.log(`power ${['light', 'normal', 'heavy'][i]}: heads=${s.heads} tails=${s.tails}`);
});
