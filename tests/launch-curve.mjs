...
// 2) 多种延迟下：不倒退（无卡顿）、有效、领先、收敛
for (const d of [0, 0.1, 0.4, 0.8]) {
  const tau = launchTau(d);
  let minSlope = 1;
  for (let t = LAUNCH_LIFT; t <= LAUNCH_LIFT + 4 * tau; t += 0.004) {
    minSlope = Math.min(minSlope, (launchFlightTime(t + 0.004, d) - launchFlightTime(t, d)) / 0.004);
  }
  assert(minSlope > -0.05, `d=${d}: 飞行时间变化率下限 ${minSlope.toFixed(3)}（不倒退=无卡顿）`);
  let ok = true;
  for (let t = LAUNCH_LIFT; t <= LAUNCH_LIFT + 4 * tau; t += 0.002) {
    const ft = launchFlightTime(t, d);
    if (ft === null || ft < LAUNCH_LIFT - 0.02 || ft > t) { ok = false; break; }
  }
  assert(ok, `d=${d}: 飞行时间全程有效且不超出真实历史（ft ≤ t）`);
  const te = LAUNCH_LIFT + 4 * tau;
  const end = launchFlightTime(te, d);
...
assert(/launchFlightTime\(t, L\.delay\)/.test(sceneSrc), 'scene.js 飞行段消费时间轴');
