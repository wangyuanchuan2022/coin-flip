...
// 4) 相机定格接线契约：held 定格 + 3 秒展示窗口 + 同步复位（硬币/黄圈/提示/相机）
...
assert(/Math\.max\(200, 3000 - scene\.renderDelay \* 1000\)/.test(mainSrc), '硬币回中在结算可见 3 秒（补偿画面延迟）');
assert(/scene\.hideRing\(\);[^\n]*\n\s*ui\.hideResult\(\)/.test(mainSrc), '硬币回中同步撤黄圈与结果提示');
assert(/setRestLook\(physics\.coinBody\.position\)/.test(mainSrc), '相机回位注视点同步改为中心硬币位置');
const uiSrc = fs.readFileSync(new URL('../src/ui.js', import.meta.url), 'utf8');
assert(/hideResult\(\)/.test(uiSrc), 'ui.js 提供 hideResult');
